	...
	<view class="page">
		...
		<view class="main">
			...
			<view class="main-no" v-if="recordList && recordList.length == 0">
				...
				<view class="no-title">暂无充值记录</view>
			</view>

			<view class="bottom" v-else>
				<view class="bottom-item" v-for="(item,index) in recordList" :key="index">
					<!-- 充值 detailStatus = 0 -->
					<block v-if="item.detailStatus == 0">
						<block v-if="item.rechargeStatus == 0">
							<view class="item-left">
								<view class="left-top-fail" v-if="item.rechargeType == 1">微信充值-失败</view>
								<view class="left-top-fail" v-if="item.rechargeType == 2">银联充值-失败</view>
								<view class="left-bottom">{{item.createTime}}</view>
							</view>
							<view class="item-right-add-fail" v-if="!factory.isHaveStr(item.detailAmount, '-')">+{{item.detailAmount}}</view>
							<view class="item-right-fail" v-else> {{item.detailAmount}}</view>
						</block>
						<block v-if="item.rechargeStatus == 1">
							<view class="item-left">
					...
					<block v-if="item.detailStatus == 1">
						<view class="item-left">
							<view class="left-top" v-if="item.payType == 1">{{item.serverClassificationName}}加工定金</view>
							<view class="left-top" v-if="item.payType == 2">{{item.serverClassificationName}}加工尾款</view>
							<view class="left-bottom">{{item.createTime}}</view>
						</view>
						<!-- 						<view class="item-right-add" v-if="!factory.isHaveStr(item.detailAmount, '-')">+{{item.detailAmount}}</view> -->
					...
					<block v-if="item.detailStatus == 2">
						<view class="item-left">
							<view class="left-top">{{item.serverClassificationName}}加工定金退款</view>
							<view class="left-bottom">{{item.createTime}}</view>
						</view>
						<!-- 						<view class="item-right-add" v-if="!factory.isHaveStr(item.detailAmount, '-')">+{{item.detailAmount}}</view> -->
						<view class="item-right-add"> +{{item.detailAmount}}</view>
