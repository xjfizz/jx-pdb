<template>
	<view class="count-box" :class="status ? 'count-box-light' : 'count-box-gray'">
		...
		<input type="number" v-model="myValue" @input="countInput" @focus="onFocus" @blur="onBlue" class="count-input uni-input" cursor-spacing="30" />
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			// 校验数量
			countInput() {
				if(this.myValue == null || this.myValue == '' || isNaN(this.myValue) || !(/^\d+(?=\.{0,1}\d+$|$)/.test(this.myValue))) {
					this.$nextTick(() =>{
						this.myValue = ''
					})
					return app.globalData.showToast('必须为大于0的整数')
				}
			},
			onBlue() {
